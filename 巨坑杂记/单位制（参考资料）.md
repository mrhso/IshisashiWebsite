...
_此处电流强度（current）、电流密度（current density）、电感（inductance）的 Gauss 单位取电磁单位而非静电单位；电阻（resistance）的 Gauss 单位既列出按 $\dfrac{V_\text{ESU}}{I_\text{EMU}}$ 定义的单位，也列出纯静电单位；电导率（conductivity）只取按 $\dfrac{I_\text{EMU}}{SE_\text{ESU}}$ 定义的单位。_
