...
在第 30 章中，可以看到 Werckmeister III、Werckmeister IV、Werckmeister V 三类良律。表中给出了一系列音程与纯五度 $\dfrac{3}{2}$ 的偏差，以及另一系列音程与纯大三度 $\dfrac{5}{4}$ 的偏差。分析纯五度一栏可知使用的音差是毕氏差 $\dfrac{531441}{524288}$。不过，毕氏差 $\dfrac{531441}{524288}$ 与合音差 $\dfrac{81}{80}$ 十分相近，由 $\dfrac{5}{4}\dfrac{81}{80}\left(\dfrac{531441}{524288}\right)^{-1}$ 可得毕氏减四度 $\dfrac{8192}{6561}$。倒不如说表中的所谓「纯大三度」其实是毕氏减四度。
