...
1967 年 10 月，CGPM 第 13 次会议[第 1 号决议](https://www.bipm.org/fr/committees/cg/cgpm/13-1967/resolution-1)修改了 SI 中秒的定义。自此，SI 中秒便是以铯-133 原子基态超精细能级跃迁频率 $\Delta\nu_\text{Cs}$ 定义的，也就是秒的现代定义。
...
1958 年，英国国家物理实验室与美国海军天文台的联合实验[结果](http://www.leapsecond.com/history/1958-PhysRev-v1-n3-Markowitz-Hall-Essen-Parry.pdf)表明，一个历书秒相当于铯-133 基态超精细能级跃迁的 9192631770 个周期（误差是 ±20 周期）。这就为 SI 秒的新定义打下了实验基础。
...
结果 1967 年 10 月 [CGPM 第 13 次会议](https://www.bipm.org/fr/committees/cg/cgpm/13-1967)上并没有同意这个要求——决议里压根没提到「历书秒」这个词，而是说「目前生效的定义」。BIPM 局长 Jean Terrien [后来指出](https://iopscience.iop.org/article/10.1088/0026-1394/8/3/004)（见 103 页左下方至右上方）BIPM 并没有做时间和频率测量的实验工作（因此他本人也加入了 IAU 第 31 委员会），并且认为这方面天文学家和物理学家之间有着误解。
...
1970 年 6 月，[CCDS 第 5 次会议](https://www.bipm.org/fr/committees/cc/cctf/5-1970)提出了国际原子时（法 Temps atomique international, TAI、英 International Atomic Time）的定义：TAI 是 BIH 根据各机构运行的原子钟读数（以 SI 的时间单位秒为基础）综合确定的时间参考坐标。

TAI 的定义中明确了统一的秒长，即 SI 秒。

1971 年 10 月 CGPM 第 14 次会议[第 1 号决议](https://www.bipm.org/fr/committees/cg/cgpm/14-1971/resolution-1)指出，CGPM 批准 CIPM 定义 TAI，定义与 BIH 合作实施。
...
20 世纪 60 年代，美国和德国的长波无线电广播时间信号使用步进原子时（英 Stepped Atomic Time, SAT）。秒长采用原子秒长，依时机插入 100 毫秒或 200 毫秒的间隔以与 UT2 同步。

1966 年 [CCIR 第 374-1 号建议](https://search.itu.int/history/HistoryDigitalCollectionDocLibrary/4.277.43.en.1003.pdf)（见 281 页）参考了这种做法，并且规定了具体细节。例如，铯频率 $f(\text{Cs})=9192631770\kern{3 mu}\text{Hz}$ 取作时间间隔测量标准；与 UT2 的差异应控制在 100 毫秒以内；载波频率与时间信号速率均不偏移时，调整时机为每月第一日，调整量为 100 毫秒的整数倍。
