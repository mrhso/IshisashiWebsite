...
1928 年 IAU 规定，将日期和当天时刻均从午夜算起的 GMT 称为世界时（Universal Time, UT）；UT 又可称为 Greenwich 民用时间（Greenwich Civil Time, GCT）——但夏令时一实施的话，就会出现「Greenwich 民众使用的时间不是 GCT」的奇怪场景，所以这一条到 1935 年又废止了。德语可将 UT 称为 Weltzeit，缩写为 WZ。
...
到[2004 年 8 月](https://hpiers.obspm.fr/iers/bul/bulb/explanatory.pdf)又删了，可能是觉得之前给的算法系数精度有些低。不过……[美国海军天文台某网页](https://maia.usno.navy.mil/information/eo-values)还留着呢。
