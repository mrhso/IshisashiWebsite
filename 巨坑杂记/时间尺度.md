...
历史上也确实使用算法乙而非算法甲。例如，[Bulletin horaire 系列 J 第 1 号](https://bibnum.obspm.fr/ark:/11287/27dZS)前面的 1966 年频率偏移公告中就出现了「秒信号相隔 $(1+300\times10^{-10})$ 原子秒」的字样（相对频率偏移为 $-300\times10^{-10}$）。
...
$(\text{MJD}\_\text{UTC literal},n)=(\lfloor\text{MJD}\_\text{UTC quasi}\rfloor+\dfrac{24\kern{3 mu}\text{h}-s(\lfloor\text{MJD}\_\text{UTC quasi}\rfloor+1)}{24\kern{3 mu}\text{h}}(\text{MJD}\_\text{UTC quasi}-\lfloor\text{MJD}\_\text{UTC quasi}\rfloor),\lfloor\text{MJD}\_\text{UTC quasi}\rfloor)$
