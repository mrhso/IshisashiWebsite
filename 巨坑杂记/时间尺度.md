...
值得注意的是，[Bulletin horaire 系列 4 第 4 号](https://bibnum.obspm.fr/ark:/11287/3wsDZ)给出的公式（见 79 页，另可见[重排版](https://www.ucolick.org/~sla/leapsecs/BHs4n4.pdf)）和上面写的并不一样。
...
先放一下 [Bulletin horaire 系列 4 第 4 号](https://bibnum.obspm.fr/ark:/11287/3wsDZ)中规定的 1956 年用公式（见 81 页，另可见[重排版](https://www.ucolick.org/~sla/leapsecs/BHs4n4.pdf)）。
...
1955 年 9 月 IAU 第 9 届大会上，第 31 委员会规定自 1956 年 1 月 1 日起，无线电广播时间信号应提供 UT2。可在 [Bulletin horaire 系列 4 第 4 号](https://bibnum.obspm.fr/ark:/11287/3wsDZ)找到相关内容摘录（见 77 页，另可见[重排版](https://www.ucolick.org/~sla/leapsecs/BHs4n4.pdf)）。
...
1965 年 [Bulletin horaire 系列 J 第 1 号](https://bibnum.obspm.fr/ark:/11287/27dZS)一句黑体写着的定义（见 2 页）中出现了法文 TUC 及英文 UTC 字样，是已知最早使用 UTC 一词的出版物。UTC 其实并不符合英语语序，或许是为了与 UT0、UT1、UT2 的格式保持一致。
...
设相对频率偏移为 $o$，标称频率为 $f_0=F_0\dfrac{\Delta\nu_\text{Cs}}{9192631770}$，实际频率为 $f=F\dfrac{\Delta\nu_\text{Cs}}{9192631770}$，$n$（可以不是整数）个脉冲依 TAI 与 UTC 所计的时间各为 $\Delta t_\text{TAI}$ 与 $\Delta t_\text{UTC}$。

依相对频率偏移的定义有 $f=(1+o)f_0$。

考虑到 SI 秒及频率的定义，立刻得 $\Delta t_\text{TAI}=\dfrac{n}{F}\kern{3 mu}\text{s}$。

改变发射频率而不变动标称频率是为了令接收方误将 $F$ 取作 $F_0$ 计算，于是 $\Delta t_\text{UTC}=\dfrac{n}{F_0}\kern{3 mu}\text{s}$。

$\dfrac{\Delta t_\text{TAI}}{\Delta t_\text{UTC}}=\dfrac{F_0}{F}=\dfrac{f_0}{f}=\dfrac{1}{1+o}$

[Bulletin horaire 系列 J 第 1 号](https://bibnum.obspm.fr/ark:/11287/27dZS)第 2 页便写着 $\text{TUC}=\left(1+\dfrac{\Delta f}{f}\right)\text{TA}+B$。
...
已知 UTC 为 $\text{UTC}\_0$ 时 TAI 为 $\text{TAI}\_0$。那么在该时刻至下次调整前的一段时间，自然有 $\text{TAI}-\text{TAI}\_0=\dfrac{\Delta t_\text{TAI}}{\Delta t_\text{UTC}}(\text{UTC}-\text{UTC}\_0)$。

$\text{TAI}-\text{UTC}=\text{TAI}\_0+\dfrac{\Delta t_\text{TAI}}{\Delta t_\text{UTC}}(\text{UTC}-\text{UTC}\_0)-\text{UTC}=\text{TAI}\_0-\dfrac{\Delta t_\text{TAI}}{\Delta t_\text{UTC}}\text{UTC}\_0+\left(\dfrac{\Delta t_\text{TAI}}{\Delta t_\text{UTC}}-1\right)\text{UTC}$
