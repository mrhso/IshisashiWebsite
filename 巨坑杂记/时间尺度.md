...
[IERS EOC](https://hpiers.obspm.fr/iers/bul/bulc/UTC-TAI.history) 与 [IERS RS/PC](https://maia.usno.navy.mil/ser7/tai-utc.dat) 也提供了精确到 $10^7$ 的表达式。
...
#### 1972 年以来的 UTC
1972 年以来，UTC 不再有频率偏移，而衹有步长调整，同步对象也从 UT2 改为 UT1。步长也不再采用非整数秒，目前总是 1 秒，称为闰秒。
...
### POSIX 时间（Unix 时间戳）
所谓 POSIX 就是 IEEE Std 1003 给出的一系列 API 标准。
...
1988 年 9 月 30 日，[IEEE Std 1003.1-1988](https://archive.org/details/federalinformati1511nati)发布。77 页 `time()` 的描述写着「`time()` 函数返回距纪元 §2.3 的秒数」。

那就看看 2.3 怎么写的好了。

29 页写着「纪元：协调世界时 1970 年 0 时 0 分 0 秒。参见『距纪元的秒数』」。「距纪元的秒数」见 34 页，也没什么好讲的，还是看看附录好了。
