...
$\displaystyle s'(T)=\dfrac{1}{2}\int_{t_0}^{T}\left(x_p\dfrac{dy_p}{dt}-\dfrac{dx_p}{dt}y_p\right)dt$
...
$\displaystyle s(T)=-\int_{t_0}^{T}\dfrac{X(t)\dfrac{dY(t)}{dt}-Y(t)\dfrac{dX(t)}{dt}}{1+Z(t)}dt-\left(\overset{\huge\frown}{\sigma_0N_0}-\overset{\huge\frown}{\Sigma_0N_0}\right)$

需要注意的是，天文学约定 $\overset{\huge\frown}{AB}$ 表示以地心为顶点，两边各过 $A$ 与 $B$ 两点的角的大小，称其为 $A$ 与 $B$ 相对于地心的角距离。

其中 $\sigma_0$ 是 J2000.0 时的 CIO，$\Sigma_0$ 是 GCRS 的 $x$ 零点，$N_0$ 是 J2000.0 时 CIP 赤道与 GCRS 赤道的交点。

积分常数 $s_0=\overset{\huge\frown}{\sigma_0N_0}-\overset{\huge\frown}{\Sigma_0N_0}$ 依 IERS 约定选定为 $+94\kern{3 mu}\mu\text{as}$。
...
$\tan\Delta\lambda=\tan(\lambda_i-\lambda)=\dfrac{\tan\lambda_i-\tan\lambda}{1+\tan\lambda_i\tan\lambda}=\dfrac{\alpha-\beta\tan\lambda}{\beta+\alpha\tan\lambda}$

将 $\alpha$ 与 $\beta$ 中含有 $x_p$ 与 $y_p$ 的部分展开成 Taylor 多项式，阶数够用就行。
...
$\dfrac{\alpha-\beta\tan\lambda}{\beta+\alpha\tan\lambda}=\dfrac{(x_p\sin\lambda+y_p\cos\lambda)\tan\varphi+o\left(\sqrt{x_p^2+y_p^2}\right)}{1-(x_p\cos\lambda-y_p\sin\lambda)\tan\varphi+o\left(\sqrt{x_p^2+y_p^2}\right)}$

……这一部分仍然是手算的，应该没有算错。

这样 $(x_p,y_p)\rightarrow(0,0)$ 时 $(x_p\sin\lambda+y_p\cos\lambda)\tan\varphi$ 便成为 $\tan\Delta\lambda=\dfrac{\alpha-\beta\tan\lambda}{\beta+\alpha\tan\lambda}$的——也是 $\Delta\lambda$ 的——等价无穷小，于是可以直接把它当作 $\Delta\lambda$。
...
值得注意的是，[Bulletin Horaire 1955](https://www.ucolick.org/~sla/leapsecs/BHs4n4EN.pdf) 给出的公式和上面写的并不一样。

$\Delta\lambda=\lambda_i-\lambda_m=\dfrac{1}{15}(x\sin\lambda_m-y\cos\lambda_m)\tan\varphi_m$

$\text{UT1}=\text{UT0}+\Delta\lambda$

因为当时国际时间局（BIH）的约定是西经为正，所以当中出现的 $\lambda$ 其实相当于现在的 $-\lambda$。这个 $\Delta\lambda$ 也就成了之前所述 $\Delta\lambda$ 的相反数。
...
UT2 便是在 UT1 的基础上用经验公式加偏移来处理这个问题。公式的系数倒是变过几次，最终确定的公式是下面这个。
...
UT2 现在很少使用了，不过 IERS 公告 A 至今还在开头提供 UT2 的公式。
