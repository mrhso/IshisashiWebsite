...
TIRS 的 $z$ 轴指向天球中间极（Celestial Intermediate Pole, CIP），且 $x$ 轴指向地球中间原点（Terrestrial Intermediate Origin, TIO）。
...
CIRS 的 $z$ 轴指向 CIP，且 $x$ 轴指向天球中间原点（Celestial Intermediate Origin, CIO）。
...
$\displaystyle s(T)=-\int_{t_0}^{T}\dfrac{X(t)\dfrac{dY(t)}{dt}-Y(t)\dfrac{dX(t)}{dt}}{1+Z(t)}dt-(\sigma_0N_0-\Sigma_0N_0)$
