...
而且计算时为简便起见会将 $\dfrac{o}{1+o}$ 一阶近似作 $o$，记作乙。
...
[IERS EOC](https://hpiers.obspm.fr/iers/bul/bulc/UTC-TAI.history) 与 [IERS RS/PC](https://maia.usno.navy.mil/ser7/tai-utc.dat) 也提供了算法乙精确到 $10^{-7}\kern{3 mu}\text{s}$ 的表达式。
...
另外 TT 与 TAI 的秒长设计上一致，因此两者有着相当简单的关系。
...
$\text{TDB}=\text{TCB}-L_B(\text{JD}\_\text{TCB}-T_0)86400\kern{3 mu}\text{s}+\text{TDB}\_0$（其中 $L_B=1.550519768\times10^{-8}$、$T_0=2443144.5003725$、$\text{TDB}\_0=-6.55\times10^{-5}$）
...
$\displaystyle\text{TCB}-\text{TCG}=c^{-2}\left(\left.\int_{t_0}^{T}\left(\dfrac{\|\vec v_e\|^2}{2}+U_\text{ext}(\vec x_e)\right)dt\right\|\_{T=t}+\vec v_e\cdot(\vec x-\vec x_e)\right)+O\left(c^{-4}\right)$

其中 $\vec x$ 为 BCRS 中的径矢，$\vec x_e$ 与 $\vec v_e$ 分别为地球质心在 BCRS 中的径矢与速度，$U_\text{ext}$ 为地心处为参考所计的太阳系中地球以外一切天体的 Newton 势，$t$ 为 TCB，$t_0$ 为 TCB 的 1977 年 1 月 1 日 00:00:32.184，$c$ 为真空光速。

特别地，$(\vec x,t)=(\vec x_e,t_0)$ 时有 $\text{TCB}-\text{TCG}=0$（此时主部和余项均为 0），即地心处 TCG 与 TCB 的 1977 年 1 月 1 日 00:00:32.184 相同。

余项 $O\left(c^{-4}\right)$ 的比率大致是 $10^{-16}$。
